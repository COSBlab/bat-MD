...
awk '{if(NR==1)print "#! FIELDS time s1 s2 s3 s4 s5"; else{time=$1; for(i=2;i<=NF;i+=2){s[i/2]=$i; j=i+1; if($j<$i) s[i/2]=$j} print time,s[1],s[2],s[3],s[4],s[5]}}' COLVAR_SB > SALT_BRIDGE
