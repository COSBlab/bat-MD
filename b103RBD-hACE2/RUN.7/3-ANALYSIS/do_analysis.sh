...
$MPI $GMX trjcat -f ../2-PRODUCTION/traj_comp.part*.xtc -o traj_all.xtc -cat
# 2) now we fix PBCs - print every 10 steps (only protein atoms). This will result in 10001 frames
echo 1 | $MPI $GMX trjconv -f traj_all.xtc -o traj_all_PBC.xtc -s ../1-EQUIL/topol.tpr -pbc nojump -skip 10 -n ../0-TOPO/index.ndx -e 1000000
